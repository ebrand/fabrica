...
public class EsbController : ControllerBase
{
    ...
    public EsbController(ProductDbContext context, ILogger<EsbController> logger)
    {
        _context = context;
        _logger = logger;
    }
    ...
    public async Task<ActionResult<OutboxConfig>> CreateOutboxConfig(CreateOutboxConfigDto dto)
    {
        try
        {
            ...
            const string domainName = "product";
    ...
    public async Task<ActionResult<CacheConfig>> CreateCacheConfig(CreateCacheConfigDto dto)
    {
        try
        {
            ...
            var consumerGroup = dto.ConsumerGroup ?? $"product-{dto.SourceDomain}.{dto.SourceTable}";
    ...
    // GET: api/esb/summary
    // Returns a comprehensive view of what product domain publishes and consumes
    [HttpGet("summary")]
