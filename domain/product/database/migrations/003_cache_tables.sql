...
CREATE TABLE IF NOT EXISTS cache.cache_config (
    ...
    source_domain VARCHAR(100) NOT NULL,      -- e.g., "admin", "order"
    source_schema VARCHAR(100) NOT NULL,      -- e.g., "fabrica"
    source_table VARCHAR(100) NOT NULL,       -- e.g., "user"
...
CREATE TABLE IF NOT EXISTS cache.cache (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    source_domain VARCHAR(100) NOT NULL,      -- e.g., "admin"
    source_table VARCHAR(100) NOT NULL,       -- e.g., "user"
    aggregate_id UUID NOT NULL,               -- ID from source system
    tenant_id VARCHAR(100) NOT NULL,
    last_event_type VARCHAR(100) NOT NULL,    -- e.g., "user.created"
...
-- Insert example config: Product domain listens for user events from admin domain
...
VALUES ('admin', 'fabrica', 'user', true, true, true, true, 'Cache user data from admin domain')
