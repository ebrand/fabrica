...
builder.Services.AddSingleton(sp =>
{
    ...
    return new KafkaProducerService(kafkaBootstrapServers, "customer", logger);
});
...
builder.Services.AddSingleton(sp =>
{
    ...
    return new KafkaConsumerService(kafkaBootstrapServers, "customer", logger);
});
...
builder.Services.AddSingleton(sp =>
{
    ...
    return new TelemetryService(kafkaBootstrapServers, "customer", logger);
});
...
app.MapGet("/health", () => Results.Ok(new { status = "healthy", service = "customer-domain-service" }));
...
static async Task<string> GetDatabaseConnectionStringAsync(IConfiguration configuration)
{
    ...
    try
    {
        ...
        var configUrl = $"{aclAdminUrl}/api/config/database/customer";
    ...
    catch (Exception ex)
    {
        ...
        var fallbackConnString = configuration.GetConnectionString("CustomerDb")
